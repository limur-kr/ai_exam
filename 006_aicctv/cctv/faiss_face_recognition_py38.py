...
class FaceRecognitionFAISS:
    ...
    def predict_numpy(self, frame: np.ndarray) -> str:
        """
        OpenCV numpy 배열(BGR)에서 얼굴을 검출·인코딩하여 FAISS 검색 후 이름을 반환한다.

        Parameters
        ----------
        frame : OpenCV BGR numpy 배열 (cv2.VideoCapture.read()의 두 번째 반환값)
        ...
        # OpenCV BGR → face_recognition(dlib)이 요구하는 RGB로 변환
